...
uniform float u_FilmGrainSpeed;
...
void main()
{
	...
	// Film Grain
    float x = (v_vTexcoord.x + 4.0 ) * (v_vTexcoord.y + 4.0 ) * (u_FilmGrainSpeed * 10.0);
	vec4 grain = vec4(mod((mod(x, 13.0) + 1.0) * (mod(x, 123.0) + 1.0), 0.01)-0.005) * u_FilmGrainStrength;
    	
	gl_FragColor = (blend + grain) * u_AmbientShadow;
